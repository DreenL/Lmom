...
def cdfgno(x,para):
    ...
    Y = (x-U)/A
    if G==0:
        CDFGNO = 0.5+0.5*sp.erg(Y*1/np.lib.scimath.sqrt(2))
    else:
        Arg = 1-G*Y

        if Arg > SMALL:
            Y = -np.lib.scimath.log(Arg)/G
            CDFGNO = 0.5+0.5*spsp.erf(Y*1/np.lib.scimath.sqrt(2))
        elif G<0:
            CDFGNO = 0
        else:
            CDFGNO = 1
...
def cdfnor(x,para):
    if para[1] < 0:
        print("Invalid Parameters")
    cdfnor = 0.5+0.5*spsp.erf((x-para[0])/para[1]*1.0/np.lib.scimath.sqrt(2))
    return(cdfnor)
...
def cdfpe3(x,para):
    ...
    if para[1]<= 0:
        ...
        if abs(Gamma) <= SMALL:
            Z = (x-para[0])/para[1]
            CDFPE3 = 0.5+0.5*spsp.erf(Z*1/np.lib.scimath.sqrt(2))
            return(CDFPE3)
        else:
            Alpha = 4/(Gamma**2)
...
def cdfwak(x,para):
    ...
    if C == 0:
        ...
        if x >= (XI+A/B):
            return(CDFWAK)
        Z = -np.lib.scimath.log(1-(x-XI)*B/A)/B
        if -Z >= UFL:
            CDFWAK = 1-np.exp(-Z)
        return(CDFWAK)
    ...
    if A == 0:
        Z = np.lib.scimath.log(1+(x-XI)*D/C)/D
        if -Z >= UFL:
            CDFWAK = 1-np.exp(-Z)
        return(CDFWAK)
    ...
    if x < quawak.quawak(0.99,para):
        ...
        if D < 0:
            Z = np.lib.scimath.log((x-XI-A/B)*D/C+1)/D
        if D == 0:
            Z = (x-XI-A/B)/C
        if D > 0:
            Z = np.lib.scimath.log((x-XI)*D/C+1)/D
...
def lmrgam(para,nmom):
    ...
    xmom.append(Beta*1/np.lib.scimath.sqrt(sp.pi)*np.exp(spsp.gammaln(Alpha+0.5)-spsp.gammaln(Alpha)))
    if nmom == 2:
        return(xmom)
    ...
    if Alpha < 1:
        ...
        Z=1/Alpha
        xmom.append(np.lib.scimath.sqrt(Z)*(((A3*Z+A2)*Z+A1)*Z+A0)/((B2*Z+B1)*Z+1))
        if nmom == 3:
            return(xmom)
...
def lmrgno(para,nmom):
    ...
    RRT2 = 1/np.lib.scimath.sqrt(2)
    RRTPI = 1/np.lib.scimath.sqrt(sp.pi)
...
def lmrkap(para,nmom):
    ...
    if ICASE == 1:
        ...
        for IR in range(1,nmom+1):
            ARG = DLGAM + spsp.gammaln(-IR/H-G) - spsp.gammaln(-IR/H)-G*np.lib.scimath.log(-H)
            if abs(ARG) > OFL:
        ...
        Beta = []
        for IR in range(1,nmom+1):
            Beta.append(np.exp(DLGAM-G*np.lib.scimath.log(IR))*(1-0.5*H*G*(1+G)/IR))

    elif ICASE == 3:
        Beta = []
        for IR in range(1,nmom+1):
            ARG = DLGAM+ spsp.gammaln(1+IR/H)-spsp.gammaln(1+G+IR/H)-G*np.lib.scimath.log(H)
            if abs(ARG) > OFL:
                print("Calculation of L-Moments Failed")
                return
            Beta.append(np.exp(ARG))
        ...
        Beta = []
        for IR in range(1,nmom+1):
            Beta.append(EU+np.lib.scimath.log(-H)+spsp.psi(-IR/H))
            
    elif ICASE == 5:
        Beta = []
        for IR in range(1,nmom+1):
            Beta.append(EU+np.lib.scimath.log(IR))

    elif ICASE == 6:
        Beta = []
        for IR in range(1,nmom+1):
            Beta.append(EU+np.lib.scimath.log(H)+spsp.psi(1+IR/H))
...
def lmrpe3(para,nmom):
    SMALL = 1e-6
    CONST = 1/np.lib.scimath.sqrt(sp.pi)
    A0 = 0.32573501
    [A1,A2,A3] = [0.16869150, 0.078327243,-0.0029120539]
    [B1,B2] = [0.46697102,0.24255406]
    C0 = 0.12260172
...
def pelgev(xmom):
    ...
    maxit = 20
    EU =0.57721566
    DL2 = np.lib.scimath.log(2)
    DL3 = np.lib.scimath.log(3)
    A0 =  0.28377530
    A1 = -1.21096399
    A2 = -2.50728214
    A3 = -1.13455566
    ...
    if T3<= 0:
        ...
        if T3 <= -0.97:
            G = 1-np.lib.scimath.log(1+T3)/DL2
...
def pelgno(xmom):
    ...
    if abs(T3)<= SMALL:
        para =[xmom[0],xmom[1]*np.lib.scimath.sqrt(sp.pi),0] 
...
def pelgum(xmom):
    ...
    if xmom[1] <= 0:
        ...
        para2 = xmom[1]/np.lib.scimath.log(2)
        para1 = xmom[0]-EU*para2
        para = [para1, para2]
        return(para)
    ...
    for it in range(1,MAXIT+1):
        ...
        if DIST < EPS:
            ...
            if TEMP > OFLEXP:
                ...
                return
            GAM = np.exp(TEMP)
            TEMP = (1+G)*np.lib.scimath.log(abs(H))
            if  TEMP > OFLEXP:
                print("Failed to converge")
                return
...
def pelnor(xmom):
    if xmom[1] <= 0:
        ...
        para = [xmom[0],xmom[1]*np.lib.scimath.sqrt(sp.pi)]
        return(para)
...
def pelpe3(xmom):
    ...
    if T3<= Small:
        para = []
        para.append(xmom[0])
        para.append(xmom[1]*np.lib.scimath.sqrt(sp.pi))
        para.append(0)
        return(para)
    ...
    RTALPH=np.lib.scimath.sqrt(Alpha)
    BETA=np.lib.scimath.sqrt(sp.pi)*xmom[1]*np.exp(sp.special.gammaln(Alpha)-sp.special.gammaln(Alpha+0.5))
    para = []
    para.append(xmom[0])
...
def pelwei(lmom):
    ...
    if lmom[1] <= 0 or lmom[2] >= 1:
        print("L-Moments Invalid")
        return
    pg = pelgev([-lmom[0],lmom[1],-lmom[2]])
    delta = 1/pg[2]
    beta = pg[1]/pg[2]
...
def quaexp(F,para):
    ...
    QUAEXP = U-A*np.lib.scimath.log(1-F)
    return(QUAEXP)
...
def quagam(F,para):
    ...
    if AM1 != 0:
        ...
        if AM1 <= 0:
            ...
            Root = Alpha*(1-1/(9*Alpha) + quastn(F)/np.lib.scimath.sqrt(9*Alpha))**3

        if (Root <= 0.01*Alpha).any():
            Root = np.exp((np.lib.scimath.log(Alpha*F)+DLOGG)/Alpha)
        ...
        QUAGAM = -np.lib.scimath.log(1-F)*Beta
        return(QUAGAM)
...
def quagev(F,para):
    ...
    if (F <= 0).any() or (F >= 1).any():
        ...
        Y = -np.lib.scimath.log(-np.lib.scimath.log(F))
        if G != 0:
            Y = (1-np.exp(-G*Y))/G
        QUAGEV = U+A*Y
        return(QUAGEV)
...
def quagum(F,para):
    ...
    if (F <= 0).any() or (F >= 1).any():
        print("F Value Invalid")
        return
    QUAGUM = U-A*np.lib.scimath.log(-np.lib.scimath.log(F))
    return(QUAGUM)
    ...
    if F <= 0 | F>= 1:
        if F==0:
            ...
            if H > 0 and G!= 0:
                QUAKAP = U+A/G*(1-H**(-G))
            if H > 0 and G == 0:
                QUAKAP = U+A*np.lib.scimath.log(H)
        ...
        Y = -np.lib.scimath.log(F)
        if H!=0:
            Y = (1-np.exp(-H*Y))/H
            
        Y = -np.lib.scimath.log(Y)
        if G!= 0:
            Y = (1-np.exp(-G*Y))/G
        QUAKAP = U+A*Y
        return(QUAKAP)
...
def quastn(F):
    ...
    if (abs(Q) > split1).any():
        ...
        if (R <= 0).any():
            print("F Value Invalid")
        R = np.lib.scimath.sqrt(-np.lib.scimath.log(R))
        if (R > split2).any():
            R = R - split2
...
def quawei(f,para):
    ...
    if para[1] <= 0 or para[2] <= 0:
        print("Invalid Parameters")
    if f.any() <0 or f.any() > 1:
        print("F Value Invalid")

    return(para[0]+para[1]*((-np.lib.scimath.log(1-f))**(1/para[2])))
